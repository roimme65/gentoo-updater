...
if command -v gh &> /dev/null; then
    ...
    if gh discussion --help 2>&1 | grep -q "unknown command"; then
        print_warning "GitHub Discussion nicht unterstützt (gh CLI zu alt)"
        print_info "→ Erstelle Discussion manuell: https://github.com/roimme65/gentoo-updater/discussions/new"
    else
        # Versuche die Discussion zu erstellen
        discussion_output=$(gh discussion create \
            --title "v${NEW_VERSION} - Release Discussion" \
            --body "🎉 **v${NEW_VERSION} ist jetzt verfügbar!**
...
Bitte benutzt diesen Thread um v${NEW_VERSION} zu diskutieren." \
        ...
        if [ $discussion_exit_code -eq 0 ]; then
            ...
            if echo "$discussion_output" | grep -q "unknown command"; then
                print_warning "GitHub Discussion nicht unterstützt (gh CLI zu alt)"
                print_info "→ Erstelle Discussion manuell: https://github.com/roimme65/gentoo-updater/discussions/new"
            else
